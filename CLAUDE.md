...
- `.github/workflows/deploy.yml`, `.github/environments/` and `.github/scripts/arm-environments.sh` — the last stage. There is nowhere to deploy to, so "deploy" here means what it honestly can: after a human approves, the tag `production` starts pointing at one image digest whose build provenance has been verified. Only a `*-v*` release tag may be promoted, never a branch. Run `arm-environments.sh` once to create the environment and its reviewer — until then GitHub creates the environment implicitly with no protection at all, and the workflow runs straight through, which looks exactly like success. See `.claude/skills/gh-deploy-env/SKILL.md`, which also lists what is waiting on there being a real target.
...
- `release-plz.toml` and `.github/workflows/release.yml` — the release pipeline. Each crate carries **its own version number**, so a change to one service releases only that service. Raise the version in that crate's `Cargo.toml` in the pull request that earns it — that is the release decision, and it is a human one — and merging produces a per-crate git tag such as `worker-service-v0.2.0`, a GitHub Release, and a container image tagged with that version. There is deliberately no automatic release pull request: deciding the next version requires `cargo package`, which resolves every dependency from a registry, and the three internal libraries exist nowhere but this repository. Nothing is published to any registry: the workspace declares `publish = ["internal"]`, and no registry by that name exists, so `cargo publish` is refused. It is deliberately not `publish = false` — release-plz skips an unpublishable package entirely, tags included. See `.claude/skills/rust-release/SKILL.md`, which also covers the one piece of manual setup: the `RELEASE_PLZ_TOKEN` secret, without which the release pull request gets no status checks and can never be merged.
